...
void GameSettingsManager::SetFullscreen(bool enabled) { { std::lock_guard<std::mutex> lock(m_mutex); m_launchWindowed.reset(); m_settings.fullscreen = enabled; MarkDirty(); } WindowManager::SetFullscreen(enabled); }
...
bool GameSettingsManager::GetFullscreen() const { std::lock_guard<std::mutex> lock(m_mutex); return m_launchWindowed.has_value() ? !m_launchWindowed.value() : m_settings.fullscreen; }
